...
from bah2026.config import (
    ...
    SOLEXS_ENERGY_KEV, SOLEXS_CHANNELS, SOLEXS_CADENCE_SEC, SOLEXS_ROWS_PER_DAY,
    ...
    FEATURE_SPECTRAL_ENTROPY_NPERSEG, FEATURE_AUTOCORR_LAGS, FEATURE_PERCENTILES,
    FORECAST_TRAIN_RATIO, FORECAST_VAL_RATIO, FORECAST_TEST_RATIO,
    ...
    CNNLSTM_INPUT_LEN, CNNLSTM_N_CHANNELS, CNNLSTM_LR,
    N_WORKERS,
    ensure_output_dirs, load_config, save_default_config,
)
...
def test_solexs_constants():
    ...
    assert SOLEXS_TO_GOES_SCALE == 1e-8
...
def test_forecast_params():
    ...
    assert abs(FORECAST_TRAIN_RATIO + FORECAST_VAL_RATIO + FORECAST_TEST_RATIO - 1.0) < 0.01
