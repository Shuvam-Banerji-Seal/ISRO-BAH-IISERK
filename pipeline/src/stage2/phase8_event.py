...
EVT = Path("data/raw/hel1os/20260623/events/evt.fits")
...
def extract():
    ...
    hxr_smooth = signal.savgol_filter(np.nan_to_num(hxr_flux, nan=0), 11, 2, mode="constant")
    short_window = 1
    long_window = 5
    hxr_short = np.convolve(np.nan_to_num(hxr_flux, nan=0), np.ones(short_window)/short_window, mode="same")
    hxr_long = np.convolve(np.nan_to_num(hxr_flux, nan=0), np.ones(long_window)/long_window, mode="same")
    ...
    if np.isnan(hxr_sigma) or hxr_sigma == 0:
        hxr_sigma = 1.0
    binary_trigger = ((trigger_ratio > 2.0) | (hxr_flux > hxr_bg + 5 * hxr_sigma)).astype(np.int8)
    ...
    for i in range(half_w, n - half_w):
        ...
        if i % 21600 == 0:
            print(f"    Cross-corr: t={i}s, max_corr={corr_max[i]:.3f} @ lag={lag_max[i]:.0f}s")
    ...
    try:
        with fits.open(EVT) as h:
            ...
            czt2 = h[4].data
        if len(czt1) > 0:
            mjd = np.concatenate([czt1["mjd"].astype(np.float64), czt2["mjd"].astype(np.float64)])
            pix = np.concatenate([czt1["pix"].astype(int), czt2["pix"].astype(int)])
            ener = np.concatenate([czt1["ener"].astype(np.float64), czt2["ener"].astype(np.float64)])
